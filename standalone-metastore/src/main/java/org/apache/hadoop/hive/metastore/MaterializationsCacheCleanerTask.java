...
public class MaterializationsCacheCleanerTask implements MetastoreTaskThread {
  ...
  public long runFrequency(TimeUnit unit) {
    return MetastoreConf.getTimeVar(conf, MetastoreConf.ConfVars.MATERIALIZATIONS_INVALIDATION_CACHE_CLEAN_FREQUENCY, unit);
  }
  ...
  public void run() {
    long removedCnt = MaterializationsInvalidationCache.get().cleanup(System.currentTimeMillis() -
        MetastoreConf.getTimeVar(conf, MetastoreConf.ConfVars.MATERIALIZATIONS_INVALIDATION_CACHE_EXPIRY_DURATION, TimeUnit.MILLISECONDS));
    if (removedCnt > 0) {
