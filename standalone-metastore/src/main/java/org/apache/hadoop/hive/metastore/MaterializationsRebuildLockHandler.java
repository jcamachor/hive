...
public class MaterializationsRebuildLockHandler {
   ...
   * @param fullyQualifiedName the fully qualified name of the materialization
   * @param txnId the transaction id for the rebuild
   * @return the response to the lock request
   */
  public LockResponse lockResource(String fullyQualifiedName, long txnId) {
    final ResourceLock prevResourceLock = locks.putIfAbsent(fullyQualifiedName,
   ...
   */
  public boolean readyToCommitResource(String fullyQualifiedName, long txnId) {
   ...
   */
  public boolean refreshLockResource(String fullyQualifiedName, long txnId) {
    final ResourceLock prevResourceLock = locks.get(fullyQualifiedName);
    if (prevResourceLock == null || prevResourceLock.txnId != txnId ||
   ...
   * @throws MetaException
   */
  public boolean unlockResource(String fullyQualifiedName, long txnId) {
    final ResourceLock prevResourceLock = locks.get(fullyQualifiedName);
    if (prevResourceLock == null || prevResourceLock.txnId != txnId) {
      return false;
    }
