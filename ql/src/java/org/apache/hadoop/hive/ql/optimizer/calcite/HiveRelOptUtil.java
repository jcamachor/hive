...
public class HiveRelOptUtil extends RelOptUtil {
   ...
   */
  public static Triple<Boolean, Pair<ImmutableBitSet, ImmutableBitSet>, List<RexNode>> extractPKFKJoin(
    ...
    final Triple<Boolean, Pair<ImmutableBitSet, ImmutableBitSet>, List<RexNode>> cannotExtract =
        Triple.of(false, null, null);
      ...
      for (RelReferentialConstraint constraint : constraints) {
        if (constraint.getTargetQualifiedName().equals(nonFkTableQName)) {
          ...
          if (allContained) {
            ...
            // 4.2) Return result
            return Triple.of(true, Pair.of(lBitSet.build(), rBitSet.build()), residualPreds);
          }
        }
      }
    }
  ...
  public static Pair<Boolean, List<RexNode>> isRewritablePKFKJoin(Join join,
