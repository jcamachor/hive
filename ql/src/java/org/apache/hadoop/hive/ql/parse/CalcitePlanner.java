...
import org.apache.calcite.rex.RexUtil.FixNullabilityShuttle;
...
import org.apache.hadoop.hive.ql.parse.type.HiveFunctionRegistry;
import org.apache.hadoop.hive.ql.parse.type.HiveFunctionRegistry.AggregateInfo;
...
public class CalcitePlanner extends SemanticAnalyzer {
  ...
  private class CalcitePlannerAction implements Frameworks.PlannerAction<RelNode> {
    ...
    private RelNode genFilterRelNode(ASTNode filterExpr, RelNode srcRel,
      ...
      RexNode convertedFilterExpr = genRexNode(filterExpr, relToHiveRR.get(srcRel),
              outerRR, null, useCaching, cluster.getRexBuilder());
      if (RexUtil.isLiteral(convertedFilterExpr, false)
          && convertedFilterExpr.getType().getSqlTypeName() != SqlTypeName.BOOLEAN) {
      ...
      convertedFilterExpr = new CorrelationConverter(
          ImmutableList.of(
              new InputContext(srcRel.getRowType(), hiveColNameCalcitePosMap, relToHiveRR.get(srcRel))),
          outerNameToPosMap, outerRR, subqueryId).apply(convertedFilterExpr);
      ...
      RelNode filterRel = new HiveFilter(cluster, cluster.traitSetOf(HiveRelNode.CONVENTION), srcRel,
          new FixNullabilityShuttle(cluster.getRexBuilder(), RelOptUtil.getFieldTypeList(srcRel.getRowType()))
              .apply(factoredFilterExpr));
    ...
    private class CorrelationConverter extends RexShuttle {
      private final List<InputContext> inputCtxs;
      private final ImmutableMap<Integer, String> outerPosToNameMap;
      private final RowResolver outerRR;
      private final int correlatedId;

      private CorrelationConverter(List<InputContext> inputCtxs,
          ImmutableMap<String, Integer> outerNameToPosMap, RowResolver outerRR,
          int correlatedId) {
        this.inputCtxs = inputCtxs;
        this.outerPosToNameMap = outerNameToPosMap == null ?
            null : ImmutableBiMap.copyOf(outerNameToPosMap).inverse();
      ...
      public RexNode visitInputRef(RexInputRef col) {
        ...
        if (inputCtxs.size() == 1 && inputCtxs.get(0).inputRowResolver == null) {
          ic = inputCtxs.get(0);
        } else {
          int numberInputs = 0;
          for (InputContext inputCtx : inputCtxs) {
            int index = col.getIndex();
            String colName = inputCtx.positionToColumnName.get(index);
            if (colName != null) {
              ic = inputCtx;
              numberInputs++;
              if (numberInputs > 1) {
        ...
        if(ic == null) {
          ...
          RelDataType rowType;
          try {
            rowType = TypeConverter.getType(cluster, outerRR, null);
          } catch (CalciteSemanticException e) {
            throw new RuntimeException("Error converting type", e);
          }
          int index = col.getIndex() - inputCtxs.get(0).inputRowType.getFieldList().size();
          if (outerPosToNameMap.get(index) == null) {
        ...
        return cluster.getRexBuilder().makeInputRef(
            ic.inputRowType.getFieldList().get(pos).getType(), pos);
      }
    }
    ...
    private RelNode genFilterRelNode(QB qb, ASTNode searchCond, RelNode srcRel,
      ...
      if(isSubQuery) {
        RexNode convertedFilterLHS = genRexNode(searchCond, relToHiveRR.get(srcRel),
        ...
        ImmutableMap<String, Integer> hiveColNameCalcitePosMap = this.relToHiveColNameCalcitePosMap
            .get(srcRel);
        convertedFilterLHS = new CorrelationConverter(
            ImmutableList.of(
                new InputContext(srcRel.getRowType(), hiveColNameCalcitePosMap, relToHiveRR.get(srcRel))),
            outerNameToPosMap, outerRR, subqueryId).apply(convertedFilterLHS);
    ...
    private RelNode genGBRelNode(List<RexNode> gbExprs, List<AggregateInfo> aggInfoLst,
      ...
      RelNode gbInputRel = HiveProject.create(
          srcRel,
          RexUtil.fixUp(cluster.getRexBuilder(), gbChildProjLst, RelOptUtil.getFieldTypeList(srcRel.getRowType())),
          null);
    ...
    private AggregateInfo getHiveAggInfo(ASTNode aggAst, int aggFnLstArgIndx, RowResolver inputRR)
      ...
      AggregateInfo aInfo = HiveFunctionRegistry.getWindowAggregateFunctionInfo(
      ...
      if (hasGrpByAstExprs || hasAggregationTrees) {
        ...
        if (hasAggregationTrees) {
          ...
          for (ASTNode value : aggregationTrees.values()) {
            ...
            AggregateInfo aInfo = HiveFunctionRegistry.getAggregateFunctionInfo(
              isDistinct, isAllColumns, aggName, aggParameters);
    ...
    private RelNode genOBLogicalPlan(QB qb, Pair<RelNode, RowResolver> selPair,
      ...
      if (obAST != null) {
        ...
        for (int i = 0; i < obASTExprLst.size(); i++) {
          ...
          orderByNode = (ASTNode) obASTExprLst.get(i);
          nullObASTExpr = (ASTNode) orderByNode.getChild(0);
          ASTNode ref = (ASTNode) nullObASTExpr.getChild(0);
          Map<ASTNode, RexNode> astToExprNDescMap = null;
          ...
          if (ref != null && ref.getToken().getType() == HiveParser.Number) {
            ...
            if (selectOutputRR != null) {
              try {
                astToExprNDescMap = genAllRexNode(ref, selectOutputRR, cluster.getRexBuilder());
            ...
            if (orderByExpression == null) {
              astToExprNDescMap = genAllRexNode(ref, inputRR, cluster.getRexBuilder());
              orderByExpression = astToExprNDescMap.get(ref);
            }
            if (orderByExpression == null) {
    ...
    private RelNode genSelectRelNode(List<RexNode> calciteColLst, RowResolver out_rwsch,
      ...
      HiveRelNode selRel = HiveProject.create(
          srcRel,
          RexUtil.fixUp(cluster.getRexBuilder(), calciteColLst, RelOptUtil.getFieldTypeList(srcRel.getRowType())),
          columnNames);
        ...
        if (fi != null && fi.getGenericUDTF() != null) {
          ...
          if (genericUDTF != null && (selectStar = exprType == HiveParser.TOK_FUNCTIONSTAR)) {
            genRexNodeRegex(".*", null, (ASTNode) expr.getChild(0),
                columnList, null, inputRR, starRR, pos, out_rwsch, qb.getAliases(), false);
          }
        }
      }
        ...
        if(isSubQuery) {
          ...
          if (expr.getType() == HiveParser.TOK_ALLCOLREF) {
            pos = genRexNodeRegex(".*",
                ...
                expr, columnList, excludedColumns, inputRR, starRR, pos, out_rwsch, qb.getAliases(), true);
            selectStar = true;
...
//                    .isSkewedCol() : false);
      ...
      if (genericUDTF != null) {
        ...
        if (obAST != null
          ...
          outputRel = genSelectRelNode(columnList, out_rwsch, srcRel);
  ...
  protected static class InputContext {
    ...
    protected InputContext(RelDataType inputRowType, ImmutableMap<String, Integer> hiveNameToPosMap,
          RowResolver inputRowResolver) {
      this.inputRowType = inputRowType;
      this.positionToColumnName = ImmutableBiMap.copyOf(hiveNameToPosMap).inverse();
