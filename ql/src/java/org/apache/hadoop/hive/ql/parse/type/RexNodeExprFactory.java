...
public class RexNodeExprFactory extends ExprFactory<RexNode> {
  ...
  protected Object interpretConstantAsPrimitive(PrimitiveTypeInfo targetType, Object constantValue,
    ...
    if (constantValue instanceof Number || constantValue instanceof HiveNlsString) {
      try {
        ...
        if (PrimitiveObjectInspectorUtils.intTypeEntry.equals(primitiveTypeEntry)) {
          ...
          return HiveDecimal.create(constantValue.toString()).bigDecimalValue();
        }
    ...
    if (constantValue instanceof BigDecimal) {
      BigDecimal bigDecimal = (BigDecimal) constantValue;
  ...
  protected RexNode createNestedColumnRefExpr(
    ...
    RexNode rexNode = ((RexCall) expr).getOperands().get(0);//convert(fieldDesc.getDesc());
    if (rexNode.getType().isStruct()) {
      // regular case of accessing nested field in a column
      return rexBuilder.makeFieldAccess(rexNode, fieldName, true);
    } else {
  ...
  protected RexNode createFuncCallExpr(TypeInfo returnType, GenericUDF genericUDF,
    ...
    final FunctionInfo functionInfo = HiveFunctionRegistry.getFunctionInfo(funcText);
    return HiveFunctionRegistry.getRexNode(
  ...
  protected RexNode createFuncCallExpr(GenericUDF genericUDF, String funcText,
      List<RexNode> inputs) throws SemanticException {
    // 1) Function resolution and gather input types
    final FunctionInfo functionInfo = HiveFunctionRegistry.getFunctionInfo(funcText);
    List<RelDataType> argsTypes = new ArrayList<>();
    ...
    if (genericUDF instanceof SettableUDF) {
      ...
      returnType = HiveFunctionRegistry.getReturnType(functionInfo, argsTypes, rexBuilder);
    }
    // 3) Convert inputs (if necessary)
    List<RexNode> newInputs = HiveFunctionRegistry.convert(
        functionInfo, inputs, returnType, rexBuilder);
    // 4) Return Calcite function
    return HiveFunctionRegistry.getRexNode(
        funcText, functionInfo, newInputs, returnType, rexBuilder);
  }
  ...
  protected RexNode createExprsListExpr() {
    // TODO: ExprNodeColumnListDesc not handled by CBO
    throw new UnsupportedOperationException("ExprNodeColumnListDesc not handled by CBO");
  }
   ...
   */
  @Override
  protected RexNode addExprToExprsList(RexNode columnList, RexNode expr) {
    // TODO: ExprNodeColumnListDesc not handled by CBO
    throw new UnsupportedOperationException("ExprNodeColumnListDesc not handled by CBO");
  }
  ...
  protected boolean isExprsListExpr(Object o) {
    // ExprNodeColumnListDesc not handled by CBO
    return false;
  }
  ...
  protected List<RexNode> getExprChildren(RexNode expr) {
    return expr instanceof RexCall ?
        ((RexCall) expr).getOperands() : new ArrayList<>();
  }
  ...
  protected TypeInfo getTypeInfo(RexNode expr) {
    return TypeConverter.convert(expr.getType());
  }
   ...
   */
  @Override
  protected List<RexNode> rewriteINIntoORFuncCallExpr(List<RexNode> inOperands) throws SemanticException {
    return transformIntoOrAndClause(inOperands, rexBuilder);
  }
  ...
  protected static List<RexNode> transformIntoOrAndClause(List<RexNode> operands, RexBuilder rexBuilder) {
