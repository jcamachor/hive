...
public class HiveRelMdRowCount extends RelMdRowCount {
  ...
  public static Pair<PKFKRelationInfo, RexNode> constraintsBasedAnalyzeJoinForPKFK(Join join, RelMetadataQuery mq) {
    ...
    final Triple<Boolean, Pair<ImmutableBitSet, ImmutableBitSet>, List<RexNode>> leftInputResult =
        HiveRelOptUtil.extractPKFKJoin(join, joinFilters, false, mq);
    final Triple<Boolean, Pair<ImmutableBitSet, ImmutableBitSet>, List<RexNode>> rightInputResult =
    ...
    final ImmutableBitSet lBitSet = leftIsKey ? leftInputResult.getMiddle().left : rightInputResult.getMiddle().left;
    final ImmutableBitSet rBitSet = leftIsKey ? leftInputResult.getMiddle().right : rightInputResult.getMiddle().right;
    final List<RexNode> residualFilters = leftIsKey ? leftInputResult.getRight() : rightInputResult.getRight();
