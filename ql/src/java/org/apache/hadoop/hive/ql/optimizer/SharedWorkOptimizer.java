...
public class SharedWorkOptimizer extends Transform {
          ...
          for (ReduceSinkOperator discardableRsOp : parentToRsOps.get(rsParent)) {
            ...
            for (ReduceSinkOperator retainableRsOp : otherRsOps) {
              ...
              boolean mergeable = compareOperator(pctx, retainableRsOp, discardableRsOp) &&
                  compareOperator(pctx, retainableRsOp.getChildOperators().get(0), discardableRsOp.getChildOperators().get(0));
              if (!mergeable) {
                // Skip
  ...
  private static SharedResult extractSharedOptimizationInfoForRoot(ParseContext pctx,
    ...
    return extractSharedOptimizationInfo(pctx, optimizerCache, equalOp1, equalOp2, currentOp1, currentOp2,
        retainableOps, discardableOps, discardableInputOps, false);
  }
  ...
  private static SharedResult extractSharedOptimizationInfo(ParseContext pctx,
    ...
    return extractSharedOptimizationInfo(pctx, optimizerCache, retainableOpEqualParent, discardableOpEqualParent,
        retainableOp, discardableOp, new LinkedHashSet<>(), new LinkedHashSet<>(), new HashSet<>(), true);
  }
